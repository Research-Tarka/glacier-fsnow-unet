#!/usr/bin/env python3
"""DeepLabv3 baseline (ResNet50 backbone, ASPP dilation rates reduced to
2/4/6 for the 6x6 feature grid at patch_size=48). See
glacier_fsnow_unet.training.deeplab_model for the architecture adaptations.
...
`baselines.deeplab` from configs/config.yaml by default.

Two subcommands are also available to run either stage on its own (e.g. to
confirm a specific hyperparameter set without re-searching):

    hpo      Optuna search only. Writes hpo_deeplab_summary.json.
    confirm  Multi-seed confirmation only, of --lr/--weight-decay/--dropout-p
             (or baselines.deeplab.lr/weight_decay/dropout_p if not passed).
             Writes deeplab_confirmation_summary.json plus one checkpoint
             per seed.

Examples:

    python scripts/baselines/16_deeplab_baseline.py
    python scripts/baselines/16_deeplab_baseline.py hpo
    python scripts/baselines/16_deeplab_baseline.py confirm --lr 0.0009 --weight-decay 0.0002 --dropout-p 0.15

Mixed precision (AMP) and cuDNN autotuning are enabled by default on CUDA;
`--batch-size` defaults to the configured value, which can be raised freely
on a GPU with more headroom without changing the result other than through
batch-size-dependent optimisation dynamics.
"""
...
from glacier_fsnow_unet.training.deeplab_model import build_deeplabv3_glacier  # noqa: E402
...
def run_hpo(cfg, data, out_dir: Path, args, num_workers: int, batch_size: int, use_amp: bool) -> dict:
    ...
    storage_path = Path(cfg.optuna_storage) if cfg.optuna_storage else out_dir / "optuna_deeplab.db"
    ...
    def objective(trial: "optuna.Trial") -> float:
        ...
        def build_model_fn():
            return build_deeplabv3_glacier(in_channels=11, num_classes=4, dropout_p=dropout_p)

        result = train_one_config(
            data, build_model_fn=build_model_fn, forward_fn=_forward,
            lr=lr, weight_decay=weight_decay, batch_size=batch_size,
            max_epochs=trial_epochs, patience=trial_patience,
            seed=trial_seed, log_prefix=f"deeplab-trial{trial.number}",
    ...
    study = optuna.create_study(
        study_name="deeplab_hpo", storage=f"sqlite:///{storage_path}",
    ...
    with open(out_dir / "hpo_deeplab_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, default=str)
    print(f"[done] {out_dir / 'hpo_deeplab_summary.json'}")
...
def run_confirm(
    ...
    if lr is None or weight_decay is None:
        print(
            ...
            "set baselines.deeplab.lr/weight_decay in configs/config.yaml, or pass "
    ...
    for seed in range(seed_start, seed_start + n_seeds):
        ...
        result = train_one_config(
            ...
            seed=seed, log_prefix=f"deeplab-confirm-seed{seed}",
            num_workers=num_workers, dropout_p=dropout_p, use_amp=use_amp,
        )
    ...
    summary = {
        "architecture": "DeepLabv3 (ResNet50 backbone, ASPP rates 2/4/6)",
    ...
    with open(out_dir / "deeplab_confirmation_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, default=str)
    print(f"[done] {out_dir / 'deeplab_confirmation_summary.json'}")
    return 0
